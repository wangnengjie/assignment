...
console.log(oldTree);
console.log(newTree);
document.body.appendChild(render(oldTree));
let something = diff(oldTree, newTree);
console.log(something);



/**
 * @function 忘记从哪抄来的一个函数，能准确返回一个数据的类型
 * @param {any} obj 
 */
function type(obj) {
...
/**
 * @function diff入口
 * @param {Object} oldTree - 旧VDOM
 * @param {Object} newTree - 新VDOM
 * @returns {Array} patches - 补丁集
 */
function diff(oldTree, newTree) {
    let index = 0; //咦，原来索引的英文就是index啊2333
    let patches = {}; //然后patch果然就是补丁本意么……
    key_id = 0;
    traversal(oldTree, newTree, index, patches);
    return patches;
}
...
/**
 * @function 对比新旧节点的props
 * @param {Object} oldNode - 旧节点
 * @param {Object} newNode - 新节点
 * @returns {Object} propsChange
 */
...
/**
 * @function 计算listdiff新增节点造成的偏移量
 * @param {Object} newNodeInList 
 */
...
function render(element) {
    if (element === null)
        return document.createComment("注释节点");
    if (type(element) === "String") {
        return document.createTextNode(element);
    }
    let create = document.createElement(element.tagName);
    ...
    children.forEach(child => {
        if (type(child) === "Array") {
            for (let i = 0; i < child.length; i++) {
                let temp = render(child[i]);
                create.appendChild(temp);
            }
        } else {
            let temp = render(child);
            create.appendChild(temp);
        }
    })
    return create;
    ...
    if (patches.hasOwnProperty(index)) {
        ...
        switch (whatChange.change) {
            ...
            case ATTR:
                ...
                break;
            case REPLACE:
                newNode = render(whatChange.node);
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (type(children[i]) === "Array") {
            for (let k = 0; k < children[i].length; k++) {
                console.log(i+j);
                console.log(node.childNodes[i + j]);
                console.log(children[i][k]);
...
function setListDiff(node, moves) {
    ...
    moves.forEach(move => {
        if (move.change === DELETE) {
            ...
            if(empty===true){
                node.parentNode.replaceChild(render(move.node), node);
                node=render(move.node);
                empty=false;
                return;
            }
            let currentNode = node;
            ...
            for (let i = 0; i < index; i++) {
                if(!currentNode.nextElementSibling){
                    parent.appendChild(render(move.node));
                    return;
                }
                currentNode = currentNode.nextElementSibling;
            }
            node.parentNode.insertBefore(render(move.node), currentNode);
        }
    })
}